...
def adicionar_livro(titulo: str, autor: str, editora: str, categoria: Categoria, ano: int, disponivel: int, livro_status: Status):
    ...
    with conectar() as conn:
        cursor = conn.execute(
            ...
            """,
            (titulo.strip(), autor.strip(), editora.strip(), int(categoria.value), ano, int(disponivel), int(livro_status.value))
        )
        conn.commit()
